...
Then('an order event is sent to Drip for the {string} widget', function(widgetType) {
  ...
  cy.wrap(Mockclient.retrieveRecordedRequests({
    ...
    expect(body.grand_total).to.eq(isVirtualProduct(widgetType) ? 0.01 : 16.22)
    ...
    expect(item.name).to.endWith('Widget 1')
    expect(item.price).to.eq(isVirtualProduct(widgetType) ? 0.01 : 11.22)
    expect(item.product_id).to.eq('1')
    expect(item.product_variant_id).to.eq('1')
    expect(item.product_url).to.startWith('http://main.magento.localhost:3006/')
    expect(item.product_url).to.endWith('/' + (isVirtualProduct(widgetType) ? 'virtual-' : '') + 'widget-1.html')
    expect(item.quantity).to.eq(1)
    expect(item.sku).to.eq(isVirtualProduct(widgetType) ? 'virt-1' : 'widg-1')
    expect(item.taxes).to.eq(0)
    expect(item.total).to.eq(isVirtualProduct(widgetType) ? 0.01 : 11.22)
  })
})
